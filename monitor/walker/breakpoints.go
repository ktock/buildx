...
type Breakpoint interface {
	IsTarget(ctx context.Context, st llb.State, contErr error) (yes bool, description string, hitLocations []*solverpb.Range, err error) // TODO: do we need "description"?
...
func (b *Breakpoints) isBreakpoint(ctx context.Context, st llb.State, handleErr error) (bool, map[string][]*solverpb.Range, error) {
	...
	for k, bp := range b.breakpoints {
		isBreak, _, bhits, err := bp.IsTarget(ctx, st, handleErr) // TODO: pass Continue error
		if err != nil {
...
func (b *lineBreakpoint) IsTarget(ctx context.Context, st llb.State, _ error) (yes bool, description string, hitLocations []*solverpb.Range, err error) {
	...
	if len(hits) > 0 {
		...
		return true, "reached" + b.String(), ret, nil
	}
	return false, "", nil, nil
}
...
func (b *stopOnEntryBreakpoint) IsTarget(ctx context.Context, st llb.State, _ error) (yes bool, description string, hitLocations []*solverpb.Range, err error) {
	if *b {
		*b = false // stop only once
		return true, "onEntry", nil, nil
	}
	return false, "", nil, nil
}
...
func (b *onErrorBreakpoint) IsTarget(ctx context.Context, st llb.State, handleErr error) (yes bool, description string, hitLocations []*solverpb.Range, err error) {
	if handleErr == nil {
		return false, "", nil, nil
	}
	_, _, _, sources, err := st.Output().Vertex(ctx, nil).Marshal(ctx, nil)
